...
function updateMultRelationPosition(graph, seriesModel) {
  var edges = graph.edges; //大于3000个关系，不处理，避免影响效率，后续 添加根据当前缩放比例控制是否显示

  if (edges.length > 3000) return;
  var sameEdgesCount = {};
  ...
  for (var i = 0; i < edges.length; i++) {
    ...
    if (+curveness) {
      if (!sameEdgesCount[key]) sameEdgesCount[key] = [1, 1];else sameEdgesCount[key][0]++;
    } else {
      var key2 = edge.node2["id"] + '-' + edge.node1["id"];
      if (!sameEdgesCount[key] && !sameEdgesCount[key2]) sameEdgesCount[key] = [1, 1];else if (sameEdgesCount[key]) sameEdgesCount[key][0]++;else if (sameEdgesCount[key2]) sameEdgesCount[key2][0]++;
    }
  } //连线之间偏移量
  ...
  for (var i = 0; i < edges.length; i++) {
    ...
    if (sameEdgesCount[key][0] == 1 && edge.node1.id != edge.node2.id) continue;
    var offset = 0; //区分单双数处理情况
    //双数，奇数、偶数往不同方向移动
    ...
    var cos = y / Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2)); //如果是弧线，弧线中间点也得处理 
    ...
    edge.setLayout(layout); // console.log(i+"x1:"+layout[0][0]+"y1:"+layout[0][1]+"x2:"+layout[1][0]+"y2:"+layout[1][1])
...
exports.updateMultRelationPosition = updateMultRelationPosition;